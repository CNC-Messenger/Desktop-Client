...
class LoginWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        #self.ui.port.keyReleaseEvent = self.check_login
    ...
    def open_main(self):
        ...
        self.main.top_user.label_user.setText("Ram Durga Sai")
        self.main.show()
        self.close()
    ...
    def check_login(self, event):
        if event.key() == Qt.Key_Return or event.key() == Qt.Key_Enter:
            ...
            def open_main():
                # SHOW MAIN WINDOW
                self.main = MainWindow()
                self.main.top_user.label_user.setText(server_ip.capitalize())
                self.main.show()                
                self.close()

            if server_ip and port == "123456":
                self.ui.user_description.setText(f"Welcome {server_ip}!")
                self.ui.user_description.setStyleSheet("#user_description { color: #bdff00 }")
                self.ui.server_ip.setStyleSheet("#server_ip:focus { border: 3px solid #bdff00; }")
                self.ui.port.setStyleSheet("#port:focus { border: 3px solid #bdff00; }")
                QTimer.singleShot(1200, lambda: open_main())
            else:
                # SET STYLESHEET
                self.ui.server_ip.setStyleSheet("#server_ip:focus { border: 3px solid rgb(255, 0, 127); }")
                self.ui.port.setStyleSheet("#port:focus { border: 3px solid rgb(255, 0, 127); }")
                self.shacke_window()
...
class MainWindow(QMainWindow):
        ...
        add_user = [
            {
                ...
                "user_name" : "Seshu",
                "user_description" : "World's Best Debugger",
                "user_status" : "online",
                "unread_messages" : 2,
                "is_active" : False
            },
            {
                ...
                "user_name" : "Sathish",
                "user_description" : "Web Developer",
                "user_status" : "busy",
                "unread_messages" : 1,
                "is_active" : False
            },
    ...
    # SET server_ip TO MAIN WINDOW
    # ///////////////////////////////////////////////////////////////
    def set_user_and_description(self, server_ip):
        self.top_user.user_name = server_ip
        print(f"User: {server_ip} are logged!")
